...
/**
 * Return the configured Tourbi service fee percentage.
 *
 * @return float
 */
...
/**
 * Return the Tourbi per-bike hourly support rate.
 *
 * @return float
 */
function tourbi_theme_income_calculator_bike_hourly_rate() {
    $rate = (float) apply_filters(
        'tourbi_host_income_calculator_bike_hourly_rate',
        18.0
    );
...
function tourbi_theme_income_calculator_is_on_current_page() {
    ...
    if ( has_shortcode( (string) $post->post_content, 'tourbi_host_income_calculator' ) ) {
        return true;
    }

    $elementor_data = get_post_meta( $post->ID, '_elementor_data', true );

    return is_string( $elementor_data ) && false !== strpos( $elementor_data, 'tourbi_host_income_calculator' );
}
...
add_action( 'wp_enqueue_scripts', 'tourbi_theme_register_income_calculator_assets', 999 );

/**
 * Print a late-enqueued stylesheet when a builder renders the shortcode after wp_head.
 *
 * @return void
 */
...
add_action( 'wp_footer', 'tourbi_theme_income_calculator_print_late_style', 1 );
 ...
 * @return int
 */
function tourbi_theme_income_calculator_clamp_int( $value, $minimum, $maximum, $default ) {
...
/**
 * Format money labels used inside the static pricing UI.
 *
 * @param float  $value Numeric value.
 * @param string $currency_symbol Currency symbol.
 * @return string
 */
function tourbi_theme_income_calculator_money_label( $value, $currency_symbol ) {
    return $currency_symbol . rtrim( rtrim( number_format( (float) $value, 2, '.', '' ), '0' ), '.' );
}
...
function tourbi_theme_income_calculator_normalize_args( $args ) {
    $variant = isset( $args['variant'] ) && 'hero' === $args['variant'] ? 'hero' : 'full';
    ...
    if ( ! isset( $args['hours_min'] ) && isset( $args['experiences_min'] ) ) {
        $args['hours_min'] = $args['experiences_min'];
    }
    if ( ! isset( $args['hours_max'] ) && isset( $args['experiences_max'] ) ) {
        $args['hours_max'] = $args['experiences_max'];
    }
    if ( ! isset( $args['hours_default'] ) && isset( $args['experiences_default'] ) ) {
        $args['hours_default'] = $args['experiences_default'];
    }
    ...
    $price_min = tourbi_theme_income_calculator_clamp_int( $args['price_min'], 20, 10000, 20 );
    $price_max = tourbi_theme_income_calculator_clamp_int( $args['price_max'], $price_min, 10000, 300 );
    $price_step = tourbi_theme_income_calculator_clamp_int( $args['price_step'], 1, max( 1, $price_max - $price_min ), 5 );
    $price_default = tourbi_theme_income_calculator_clamp_int( $args['price_default'], $price_min, $price_max, 40 );

    $guests_min = tourbi_theme_income_calculator_clamp_int( $args['guests_min'], 1, 100, 1 );
    $guests_max = tourbi_theme_income_calculator_clamp_int( $args['guests_max'], $guests_min, 100, 24 );
    $guests_default = tourbi_theme_income_calculator_clamp_int( $args['guests_default'], $guests_min, $guests_max, 4 );

    $hours_min = tourbi_theme_income_calculator_clamp_int( $args['hours_min'], 1, 24, 1 );
    $hours_max = tourbi_theme_income_calculator_clamp_int( $args['hours_max'], $hours_min, 24, 8 );
    $hours_default = tourbi_theme_income_calculator_clamp_int( $args['hours_default'], $hours_min, $hours_max, 2 );
...
function tourbi_theme_income_calculator_control( $args ) {
    ...
    $icons = array(
        ...
        'hours'  => '<svg viewBox="0 0 24 24" focusable="false"><circle cx="12" cy="12" r="8"></circle><path d="M12 7v5l3 2"></path></svg>',
    );
...
function tourbi_theme_render_host_income_calculator( $args = array() ) {
    ...
    $service_fee = tourbi_theme_income_calculator_platform_fee();
    $bike_rate   = tourbi_theme_income_calculator_bike_hourly_rate();
    ...
    $controls = tourbi_theme_income_calculator_control(
        array(
            'name'            => 'price',
            'label'           => __( 'Experience price per person', 'torby' ),
            'aria_label'      => __( 'Experience price per person value', 'torby' ),
    ...
    $controls .= tourbi_theme_income_calculator_control(
        array(
            'name'        => 'guests',
            'label'       => __( 'Number of guests', 'torby' ),
            'aria_label'  => __( 'Number of guests value', 'torby' ),
            'instance_id' => $instance_id,
            'variant'     => $variant,
    ...
    $controls .= tourbi_theme_income_calculator_control(
        array(
            'name'        => 'hours',
            'label'       => __( 'Duration / Hours', 'torby' ),
            'aria_label'  => __( 'Duration in hours value', 'torby' ),
            'instance_id' => $instance_id,
            'variant'     => $variant,
            'minimum'     => $settings['hours_min'],
            'maximum'     => $settings['hours_max'],
            'step'        => 1,
            'default'     => $settings['hours_default'],
        )
    );

    $service_fee_label = tourbi_theme_income_calculator_percent_label( $service_fee );
    $bike_rate_label   = tourbi_theme_income_calculator_money_label( $bike_rate, $currency_symbol );
    ...
    <section
        ...
        data-platform-fee="<?php echo esc_attr( $service_fee ); ?>"
        data-bike-rate="<?php echo esc_attr( $bike_rate ); ?>"
            ...
            <?php if ( 'hero' === $variant ) : ?>
                <header class="tourbi-income-calculator__hero-head">
                    <p class="tourbi-income-calculator__eyebrow">
                        <?php esc_html_e( 'Estimate Your Earnings', 'torby' ); ?>
                    </p>
                    <h2
                        id="<?php echo esc_attr( $instance_id ); ?>-title"
                        class="tourbi-income-calculator__hero-title"
                    >
                        <?php esc_html_e( 'See the simple host math.', 'torby' ); ?>
                    </h2>
                </header>

                <section class="tourbi-income-calculator__hero-result" aria-label="<?php esc_attr_e( 'Estimated host earnings', 'torby' ); ?>">
                    <p class="tourbi-income-calculator__result-label" data-result-label>
                        <?php esc_html_e( 'Estimated Host Earnings', 'torby' ); ?>
                    </p>
                    <p class="tourbi-income-calculator__payout" data-payout aria-live="polite">
                        <?php echo esc_html( $currency_symbol . '0' ); ?>
                    </p>

                    <div class="tourbi-income-calculator__share-bar tourbi-income-calculator__share-bar--pricing" aria-label="<?php esc_attr_e( 'Tourbi pricing', 'torby' ); ?>">
                        <span>
                            <?php esc_html_e( 'Bike rate', 'torby' ); ?>
                            <b><?php echo esc_html( $bike_rate_label ); ?>/hr</b>
                        </span>
                        <span>
                            <?php esc_html_e( 'Service fee', 'torby' ); ?>
                            <b><?php echo esc_html( $service_fee_label ); ?>%</b>
                        </span>
                    </div>
                </section>
                ...
                <div class="tourbi-income-calculator__controls">
                    ...
                    <dl class="tourbi-income-calculator__breakdown tourbi-income-calculator__breakdown--hero">
                        <div>
                            <dt><?php esc_html_e( 'Gross revenue', 'torby' ); ?></dt>
                            <dd data-gross><?php echo esc_html( $currency_symbol . '0' ); ?></dd>
                        </div>
                        <div>
                            <dt><?php printf( esc_html__( 'Bike support (%s/hr)', 'torby' ), esc_html( $bike_rate_label ) ); ?></dt>
                            <dd data-bike-fee><?php echo esc_html( '-' . $currency_symbol . '0' ); ?></dd>
                        </div>
                        <div>
                            <dt><?php printf( esc_html__( 'Service fee (%s%%)', 'torby' ), esc_html( $service_fee_label ) ); ?></dt>
                            <dd data-fee><?php echo esc_html( '-' . $currency_symbol . '0' ); ?></dd>
                        </div>
                        <div class="tourbi-income-calculator__breakdown-total">
                            <dt><?php esc_html_e( 'Estimated host earnings', 'torby' ); ?></dt>
                    ...
                    <a class="tourbi-income-calculator__cta" href="<?php echo esc_url( $cta['url'] ?? home_url( '/vendor-register/' ) ); ?>">
                        <span><?php esc_html_e( 'Start Hosting', 'torby' ); ?></span>
                    ...
                    <p class="tourbi-income-calculator__disclaimer">
                        ...
                        <span><?php esc_html_e( 'Estimate only. Manual payout will be handled by Tourbi after booking review.', 'torby' ); ?></span>
                    </p>
                </div>
            <?php else : ?>
                <div class="tourbi-income-calculator__intro">
                    <p class="tourbi-income-calculator__eyebrow"><?php esc_html_e( 'Estimate Your Earnings', 'torby' ); ?></p>

                    <h2 id="<?php echo esc_attr( $instance_id ); ?>-title" class="tourbi-income-calculator__title">
                        <?php esc_html_e( 'Simple host', 'torby' ); ?>
                        <em><?php esc_html_e( 'earnings estimate.', 'torby' ); ?></em>
                    </h2>

                    <p class="tourbi-income-calculator__description">
                        <?php esc_html_e( 'Adjust the guest price, group size, and duration to estimate one hosted e-bike experience.', 'torby' ); ?>
                    </p>
                ...
                <div class="tourbi-income-calculator__controls">
                    ...
                    <p class="tourbi-income-calculator__disclaimer">
                        <?php esc_html_e( 'Estimate only. Manual payout will be handled by Tourbi after booking review.', 'torby' ); ?>
                    </p>
                </div>
            <?php endif; ?>
        </div>
...
add_shortcode( 'tourbi_host_income_calculator', 'tourbi_theme_host_income_calculator_shortcode' );
